...
이 프로젝트는 실제 운영되는 이커머스 서비스에서 마주할 수 있는 복잡한 문제들을 직접 해결하며 풀스택 개발 역량을 실무 수준으로 끌어올리는 것을 목표로 시작되었습니다. 특히, 견고한 **DB 설계**와 복잡한 **API 설계**와 대용량 트래픽 환경에서의 **동시성 제어**와 사용자의 개인정보를 안전하게 지키는 **보안 아키텍처 설계**에 대해 깊이 있게 학습하고 싶었습니다.
...
| 주문 내역 |
|:---:|
| ![주문 관리](./screenshots/05_order_info_desktop.png) |
...
### 🧹 고급 성능 최적화
- **Java 메모리 최적화**: Integer 캐싱 메커니즘 극복 + equals() 활용
- **MyBatis 캐시 제어**: 실시간 데이터 동기화를 위한 캐시 최적화
- **다층 캐싱 전략**: Cloudflare + Nginx 조합으로 API/정적자원 최적화
- **AWS 인프라 최적화**: t2.micro 환경에서 극한 메모리 효율화

### 🛡️ 체계적 서버 보안 & 운영
- **Cloudflare DDoS 보호**: Page Rules 세밀 설정으로 API/정적자원 분리
- **AWS 비용 관리**: 예산 알림 + 제로 지출 모니터링 시스템
- **자동 복구 시스템**: systemd 기반 메모리 부족 시 10초 내 자동 재시작
...
### 💳 결제 & 주문 관리
- [ ] **안전한 결제 시스템**: 포트원 테스트 버전 연동 + 웹훅 실시간 동기화 *(포트원 결제창 연동 구현 중)*
- [ ] **결제 완료 처리**: 결제 성공 후 DB 기록 및 상태 업데이트 *(구현 중)*
- [ ] **주문 상태 관리**: 결제대기→결제완료→배송중→배송완료→구매확정 전체 흐름 *(API 설계 완료, 프론트엔드 연동 구현 중)*
- [ ] **반품/취소/교환**: 관리자 및 회원 요청 처리 시스템 *(API 설계 완료, 프론트엔드 연동 구현 중)*
...
### 🔧 관리자 대시보드
- [x] **종합 비즈니스 관리**: 회원/상품/주문/결제 통합 관리
...
- **주요 기술적 도전**: 
  ...
  - 복잡한 상태 관리 (16가지 주문 상태 + 취소/반품/교환 워크플로우)
  - 동시성 제어 고도화 (낙관적 락 + 비관적 락 + Java Integer 캐싱 이슈 해결)
  - 포트원 결제 시스템 연동 (V1 API + 웹훅 처리)
  - 전화번호 AES-256 암호화 (PBKDF2 + AES)
  - JWT 이중 토큰 + Redis 블랙리스트 구현
  - 13개 테이블 트랜잭션 격리 수준 REPEATABLE_READ 적용
  - 모바일/데스크톱 반응형 디자인 구현
  - 실시간 성능 최적화 (Cloudflare 캐싱 전략으로 도메인 응답속도 개선)
  - 메모리 제약 극복 (AWS t2.micro에서 풀스택 애플리케이션 안정 운영)
